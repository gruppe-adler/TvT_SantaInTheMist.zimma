...
if (isServer && count _this == 0) then {
    [
        [([WEST] call mitm_points_fnc_getPointsCategorized),[],{_x select 1},"DESCEND"] call BIS_fnc_sortBy,
        [([EAST] call mitm_points_fnc_getPointsCategorized),[],{_x select 1},"DESCEND"] call BIS_fnc_sortBy,
        [([INDEPENDENT] call mitm_points_fnc_getPointsCategorized),[],{_x select 1},"DESCEND"] call BIS_fnc_sortBy,
        [([CIVILIAN] call mitm_points_fnc_getPointsCategorized),[],{_x select 1},"DESCEND"] call BIS_fnc_sortBy
    ] remoteExec ["mitm_points_fnc_displayPoints",0,false];
};

if (hasInterface && count _this > 0) then {
    "mitm_common_displayPoints" cutRsc ["RscTitleDisplayEmpty","PLAIN",0,false];
    ...
    [{!isNull (uiNamespace getVariable ["RscTitleDisplayEmpty",displayNull])}, {
        ...
        _columnCount = 0;
        {
            if (count _x > 0) then {
                ...
                _text = [parseText format ["<t size='1.5'>Points for <t color='%1'>%2</t>:</t>",[_side,"HTML"] call mitm_common_fnc_getSideColor,[_side] call mitm_common_fnc_getSideName],lineBreak,lineBreak];
                {
                ...
                _sumCtrl ctrlSetStructuredText (parseText format ["Total: <t color = '#00ff00'>%1</t>",[_side] call mitm_points_fnc_getPoints]);
        ...
        [{"mitm_common_displayPoints" cutRsc ["RscTitleDisplayEmpty","PLAIN",0,false];}, [], 12] call CBA_fnc_waitAndExecute;
